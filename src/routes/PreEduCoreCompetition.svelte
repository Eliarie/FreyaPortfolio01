<script lang="ts">
  ...
  // 1. TypeScript 支持声明
  let isOpen: boolean = false;  // 变量声明与类型注解
  // 添加点击状态管理
  ...
  function handleTextClick(text: string) {
    ...
    if (text === '教学创生力') {
      window.location.href = '/practice/teachingCreativity';
    } else if (text === '教育本源力') {
      window.location.href = '/practice/rootofEducation';
    } else if (text === '教育共信力') {
      window.location.href = '/practice/educationalTrust';
    } else if (text === '协同领导力') {
      window.location.href = '/practice/collaborativeLeadership';
    } else if (text === '数智创新力') {
      window.location.href = '/practice/digitalInnovation';
    } else if (text === '教育思想力') {
      window.location.href = '/practice/educationalThinking';
    }
  }
  ...
  onMount(() => {
    ...
    document.body.classList.add('competition-bg');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          }, 200); // 灯泡底座上方的装饰线条出现后的延迟
        }, 200); // 教育本源力及相贴的线条出现后的延迟
      }, 200); // 最底部的线条出现后的延迟
    }, 300); // 初始延迟
  });
  onDestroy(() => {
    ...
    document.body.classList.remove('competition-bg');
  });
</script>
...
<div class="competition-container">
  ...
  <!-- 始终显示的引用内容，点击开启后隐藏 -->
  {#if !isOpen}
  <div class="lamp-quote-block lamp-quote-large moved-quote-block">
...
<style>
  ...
  */
  ...
  /* 固定定位的悬浮提示容器，通常用于鼠标悬停时显示的气泡提示，始终在最上层且不响应鼠标事件 */
  .hover-tip-fixed {
    position: fixed;
    z-index: 99999;
    pointer-events: none;
  }
  /* 悬浮提示内容的样式，白底黑字，圆角，阴影，字体为Noto Serif SC，字号18px，加粗，内容不换行 */
  .hover-tip-box {
    background: white;
    color: black;
    border-radius: 6px;
  ...
  /* 导航栏在"competition-nav"模式下变为透明背景，所有文字变为白色，适配深色主题 */
  :global(.simple-nav.competition-nav) {
  ...
  /* 灯泡下方引用区块的外层容器，设置上下外边距、最大宽度、居中对齐和flex布局 */
  .lamp-quote-block {
    position: relative;
  ...
  /* 说明区块的外层容器，设置了上下外边距、最大宽度、字体、颜色和内边距 */
  ...
  /* 引用区块左侧的竖线样式，黄色发光，设置宽度、高度、圆角和右侧间距 */
  ...
  /* 引用大号文字的具体样式，设置字体、字号、颜色、发光和阴影效果 */
  ...
  /* 说明列表的外层容器，设置左右外边距、上下外边距、字体、颜色等 */
  .lamp-explanation-section {
    ...
    color: white;
    font-size: 1.1em;
    font-style: 'Noto Serif SC';
    font-family: 'Noto Serif SC', SimSun;
  }

  /* 说明列表ul的样式，设置字体大小、行高、颜色和外边距 */
  ...
  /* 逻辑链条区块的外层容器，设置宽度、最大宽度、字体、颜色、外边距、最小高度等 */
  .lamp-flow-logic {
    width: 100%;
    max-width: 1500px;
  ...
  /* 逻辑链条每一行的单独定位和样式，主要是左侧缩进和上下间距 */
  .lamp-flow-row1 { margin-top: 0; margin-left: 120px;}
  .lamp-flow-row2 { margin-top: 20px; margin-left: 400px;  font-size: 1.8rem; font-weight: 500; color: #ffe066; font-weight: bold; font-family: 'Noto Serif SC', SimSun; }
  .lamp-flow-row3 { margin-top: 5px; margin-left: 550px;  font-size: 1.8rem; font-weight: 500; color: #ffe066; font-weight: bold; font-family: 'Noto Serif SC', SimSun;}
  .lamp-flow-row4 { margin-top: 5px;margin-left: 750px;  font-size: 1.8rem; font-weight: 500; color: #ffe066; font-weight: bold; font-family: 'Noto Serif SC', SimSun;}
  ...
  /* 逻辑链条中箭头、横线、内联文本等的样式，分别控制颜色、大小、间距等 */
  .lamp-flow-arrow1 {
    text-align: center;
    font-size: 2.8rem;
    color: white;
  ...
  /* 当body有competition-bg类时，页面背景强制为黑色 */
  :global(body.competition-bg) {
    background: black !important;
  }
  /* 说明区块在"close-mode"状态下的特殊样式，带有半透明背景和圆角阴影 */
  .lamp-explanation-block.close-mode {
  ...
  /* 分别为引用区块、逻辑链条区块、说明区块设置底部间距 */
  .lamp-quote-block.lamp-quote-large {
    margin-bottom: 40px;
  }
  .lamp-flow-logic {
    margin-bottom: 40px;
