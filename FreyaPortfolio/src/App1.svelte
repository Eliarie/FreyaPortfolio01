<script lang="ts">
  ...
  // import Home from './routes/Home.svelte'; // 移除对 Home.svelte 的导入
      ...
      <div class="homepage-content">
        ...
        <div class="left-block">
          <div class="creative-block">
            <div class="creative-phrases">
              {#if showP1}
                <span class="phrase p1" in:scale={{ duration: 400, start: 0.5, opacity: 0 }}>
                  ⽴⾜真实场域
                </span>
              {/if}
              {#if showP2}
                <span class="phrase p2" in:scale={{ duration: 400, start: 0.5, opacity: 0 }}>
                  教育本源为根基
                </span>
              {/if}
              {#if showP3}
                <span class="phrase p3" in:scale={{ duration: 400, start: 0.5, opacity: 0 }}>
                  创新驱动
                </span>
              {/if}
              {#if showP4}
                <span class="phrase p4" in:scale={{ duration: 400, start: 0.5, opacity: 0 }}>
                  探索多元形态
                </span>
              {/if}
              {#if showP5}
                <span class="phrase p5" in:scale={{ duration: 400, start: 0.5, opacity: 0 }}>
                  幼⼉教育实践者
                </span>
              {/if}
            </div>
...
.main-btn {
  display: inline-block;
  padding: 12px 32px;           /* 按钮内边距（可调） */
  border: 1px solid #e0d9c7;    /* 按钮边框颜色（可调） */
  border-radius: 4px;           /* 按钮圆角（可调） */
  background: #fff;             /* 按钮背景色（可调） */
  color: #293241;                  /* 按钮文字颜色（可调） */
  font-size: 1rem;              /* 按钮字体大小（可调） */
  font-weight: 500;             /* 按钮字重（可调） */
  text-decoration: none;
  transition: background 0.2s, color 0.2s;
}
.main-btn:hover {
  background:#293241;             /* 悬停背景色（可调） */
  color: #fff;                  /* 悬停文字色（可调） */
}
